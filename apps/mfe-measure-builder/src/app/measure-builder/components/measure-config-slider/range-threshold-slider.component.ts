...
export class RangeThresholdSliderComponent implements OnInit {
  ...
  onRangeMinChange(event: Event): void {
    ...
    const minValue = config.minValue ?? config.min;
    const maxValue = config.maxValue ?? config.max;
    const currentMax = config.currentMax ?? maxValue;
  ...
  private determineActivePreset(): void {
    ...
    const config = this.getThresholdConfig();
    const currentValue = config.currentValue ?? 0;
  ...
  isInWarningZone(): boolean {
    ...
    const config = this.getThresholdConfig();
    const currentValue = config.currentValue ?? 0;
    const warningThreshold = config.warningThreshold ?? config.minValue ?? config.min;
    const criticalThreshold = config.criticalThreshold ?? config.maxValue ?? config.max;
  ...
  getWarningIndicatorPosition(): number {
    ...
    const minValue = config.minValue ?? config.min;
    const maxValue = config.maxValue ?? config.max;
    const warningThreshold = config.warningThreshold ?? minValue;
  ...
  getCriticalIndicatorPosition(): number {
    ...
    const minValue = config.minValue ?? config.min;
    const maxValue = config.maxValue ?? config.max;
    const criticalThreshold = config.criticalThreshold ?? maxValue;
    const range = maxValue - minValue || 1;
    const position = ((criticalThreshold - minValue) / range) * 100;
    return position;
  }
