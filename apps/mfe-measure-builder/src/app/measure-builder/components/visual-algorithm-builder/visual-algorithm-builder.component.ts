...
export class VisualAlgorithmBuilderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private renderSVG(): void {
    ...
    // Render blocks
    const blocks = this.algorithm.blocks ?? [];
  ...
  private renderBlock(svg: SVGSVGElement, block: PopulationBlock): void {
    ...
    const blockX = block.x ?? block.position.x;
    const blockY = block.y ?? block.position.y;
    const blockWidth = block.width ?? 160;
    const blockHeight = block.height ?? 80;
    const blockName = block.name ?? block.label ?? '';
    group.setAttribute('transform', `translate(${blockX},${blockY})`);
    group.setAttribute('aria-label', `${blockName}: ${block.description ?? ''}`);
    ...
    rect.setAttribute('height', blockHeight.toString());
    ...
    text.setAttribute('x', (blockWidth / 2).toString());
    text.setAttribute('y', (blockHeight / 2 + 4).toString());
  ...
  private renderConnections(svg: SVGSVGElement): void {
    ...
    this.algorithm.connections.forEach(connection => {
      const blocks = this.algorithm?.blocks ?? [];
      const fromId = connection.fromBlockId ?? connection.sourceBlockId;
      const toId = connection.toBlockId ?? connection.targetBlockId;
      const fromBlock = blocks.find(b => b.id === fromId);
      const toBlock = blocks.find(b => b.id === toId);

      if (fromBlock && toBlock) {
        this.renderConnectionLine(svg, fromBlock, toBlock, fromId, toId);
      }
    });
  }
  ...
  private renderConnectionLine(
    ...
    const fromWidth = fromBlock.width ?? 160;
    const fromHeight = fromBlock.height ?? 80;
    const toWidth = toBlock.width ?? 160;
    const toHeight = toBlock.height ?? 80;
    const fromX = (fromBlock.x ?? fromBlock.position.x) + fromWidth / 2;
    const fromY = (fromBlock.y ?? fromBlock.position.y) + fromHeight / 2;
    const toX = (toBlock.x ?? toBlock.position.x) + toWidth / 2;
    const toY = (toBlock.y ?? toBlock.position.y) + toHeight / 2;
  ...
  startBlockDrag(blockId: string, event: MouseEvent): void {
    ...
    if (block) {
      this.dragStartBlockX = block.x ?? block.position.x;
      this.dragStartBlockY = block.y ?? block.position.y;
    }
  ...
  private onMouseMove = (event: MouseEvent): void => {
    ...
    if (block) {
      const blockWidth = block.width ?? 160;
      const blockHeight = block.height ?? 80;
      newX = Math.max(0, Math.min(newX, 1200 - blockWidth));
      newY = Math.max(0, Math.min(newY, 600 - blockHeight));
