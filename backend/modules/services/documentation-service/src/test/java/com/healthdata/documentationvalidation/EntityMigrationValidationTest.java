package com.healthdata.documentationvalidation;
...
class EntityMigrationValidationTest {
    ...
    void validateAllEntitiesMatchDatabaseSchema() {
        ...
        assertTrue(report.isValid(),
                "Entity-migration validation failed for documentation-service.\n" +
                report.getDetailedMessage());
    }

    @Test
    void validateCriticalDocumentationEntities() {
        Set<EntityType<?>> entities = entityManagerFactory.getMetamodel().getEntities();

        // Filter to critical documentation entities
        Set<EntityType<?>> criticalEntities = entities.stream()
                .filter(e -> {
                    String name = e.getName();
                    return name.equals("ClinicalDocumentEntity") ||
                           name.equals("CdaDocumentEntity") ||
                           name.equals("DocumentMetadataEntity");
                })
        ...
        assertTrue(report.isValid(),
                "Critical entity-migration validation failed for documentation-service.\n" +
                report.getDetailedMessage());
    }
}
