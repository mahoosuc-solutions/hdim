package com.healthdata.ecrvalidation;
...
/**
 * Entity-migration validation for ecr-service.
 *
 ...
 *
 * This test is in a separate package (com.healthdata.ecrvalidation) to
 * avoid Spring Boot auto-scanning for @SpringBootConfiguration in com.healthdata.ecr.
 *
 * Note: ecr-service entities use schema = "ecr", so we create the schema before
...
class EntityMigrationValidationTest {
    ...
    void validateAllEntitiesMatchDatabaseSchema() {
        ...
        assertTrue(report.isValid(),
                "Entity-migration validation failed for ecr-service.\n" +
                report.getDetailedMessage());
    }

    @Test
    void validateCriticalEcrEntities() {
        Set<EntityType<?>> entities = entityManagerFactory.getMetamodel().getEntities();

        // Filter to critical ecr entities
        Set<EntityType<?>> criticalEntities = entities.stream()
                .filter(e -> {
                    String name = e.getName();
                    return name.equals("ElectronicCaseReportEntity") ||
                           name.equals("RctcTriggerCodeEntity");
                })
        ...
        assertTrue(report.isValid(),
                "Critical entity-migration validation failed for ecr-service.\n" +
                report.getDetailedMessage());
    }
}
