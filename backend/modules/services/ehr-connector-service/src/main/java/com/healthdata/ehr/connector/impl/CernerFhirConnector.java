...
/**
 * Cerner (Oracle Health) FHIR R4 connector implementation.
 * Supports Cerner's OAuth2 authentication flow.
 */
@Slf4j
public class CernerFhirConnector extends FhirConnector {

    public CernerFhirConnector(EhrConnectionConfig config, WebClient.Builder webClientBuilder) {
        super(config, webClientBuilder);
    }

    @Override
    protected Mono<String> authenticate() {
        log.debug("Authenticating with Cerner using OAuth2 client credentials");

        String tokenUrl = config.getTokenUrl() != null ?
                config.getTokenUrl() :
                config.getBaseUrl().replace("/r4", "") + "/oauth/token";
        ...
        return WebClient.create()
                ...
                .map(response -> {
                    ...
                    log.debug("Cerner authentication successful, token expires in {} seconds", expiresIn);
                    return accessToken;
                })
                .doOnError(error -> log.error("Cerner authentication failed", error));
    }
    ...
    /**
     * Generate Basic Auth header for Cerner OAuth2.
     */
