package com.healthdata.migrationvalidation;
 ...
 *
 * This test is in a separate package (com.healthdata.migrationvalidation) to
 * avoid Spring Boot auto-scanning for @SpringBootConfiguration in com.healthdata.migration.
 */
@DataJpaTest(
    properties = {
...
class EntityMigrationValidationTest {
    ...
    @EntityScan(basePackages = "com.healthdata.migration.persistence")
    ...
    void validateAllEntitiesMatchDatabaseSchema() {
        ...
        assertTrue(report.isValid(),
                "Entity-migration validation failed for migration-workflow-service.\n" +
                report.getDetailedMessage());
    }

    @Test
    void validateCriticalMigrationEntities() {
        Set<EntityType<?>> entities = entityManagerFactory.getMetamodel().getEntities();

        // Filter to critical migration entities
        Set<EntityType<?>> criticalEntities = entities.stream()
                .filter(e -> {
                    String name = e.getName();
                    return name.equals("MigrationJobEntity") ||
                           name.equals("MigrationCheckpointEntity") ||
                           name.equals("MigrationErrorEntity");
                })
        ...
        assertTrue(report.isValid(),
                "Critical entity-migration validation failed for migration-workflow-service.\n" +
                report.getDetailedMessage());
    }
}
