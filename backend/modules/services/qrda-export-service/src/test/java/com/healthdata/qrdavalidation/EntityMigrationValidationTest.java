package com.healthdata.qrdavalidation;
...
/**
 * Entity-migration validation for qrda-export-service.
 *
 ...
 *
 * This test is in a separate package (com.healthdata.qrdavalidation) to
 * avoid Spring Boot auto-scanning for @SpringBootConfiguration in com.healthdata.qrda.
 */
@DataJpaTest(
    properties = {
...
class EntityMigrationValidationTest {
    ...
    void validateAllEntitiesMatchDatabaseSchema() {
        ...
        assertTrue(report.isValid(),
                "Entity-migration validation failed for qrda-export-service.\n" +
                report.getDetailedMessage());
    }

    @Test
    void validateCriticalQrdaEntities() {
        Set<EntityType<?>> entities = entityManagerFactory.getMetamodel().getEntities();

        // Filter to critical QRDA entities
        Set<EntityType<?>> criticalEntities = entities.stream()
                .filter(e -> {
                    String name = e.getName();
                    return name.equals("QrdaExportJobEntity") ||
                           name.equals("CmsSubmissionEntity");
                })
        ...
        assertTrue(report.isValid(),
                "Critical entity-migration validation failed for qrda-export-service.\n" +
                report.getDetailedMessage());
    }
}
