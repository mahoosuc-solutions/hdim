package com.healthdata.sdohvalidation;
...
/**
 * Entity-migration validation for sdoh-service.
 *
 ...
 *
 * This test is in a separate package (com.healthdata.sdohvalidation) to
 * avoid Spring Boot auto-scanning for @SpringBootConfiguration in com.healthdata.sdoh.
 */
@DataJpaTest(
    properties = {
...
class EntityMigrationValidationTest {
    ...
    @EntityScan(basePackages = "com.healthdata.sdoh.entity")
    static class TestConfig {
    }

    @Container
    static PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16-alpine")
            .withDatabaseName("sdoh_test")
    ...
    void validateAllEntitiesMatchDatabaseSchema() {
        ...
        assertTrue(report.isValid(),
                "Entity-migration validation failed for sdoh-service.\n" +
                report.getDetailedMessage());
    }

    @Test
    void validateCriticalSdohEntities() {
        Set<EntityType<?>> entities = entityManagerFactory.getMetamodel().getEntities();

        // Filter to critical SDOH entities
        Set<EntityType<?>> criticalEntities = entities.stream()
                .filter(e -> {
                    String name = e.getName();
                    return name.equals("SdohAssessmentEntity") ||
                           name.equals("SdohRiskScoreEntity") ||
                           name.equals("HrsnScreeningSessionEntity");
                })
        ...
        assertTrue(report.isValid(),
                "Critical entity-migration validation failed for sdoh-service.\n" +
                report.getDetailedMessage());
    }
}
