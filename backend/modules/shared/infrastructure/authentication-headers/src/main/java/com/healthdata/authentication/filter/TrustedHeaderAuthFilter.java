 ...
 *
 * This version is in the authentication-headers module, which has NO @EntityScan.
 ...
 *
 * Key Features:
 * - Sets SecurityContext for Spring Security
 * - Populates UserContextHolder for audit and authorization
 * - Stores tenant IDs and roles in request attributes
 *
 * Usage:
 * Register this filter in your service's SecurityConfig:
 * <pre>
 * @Bean
...
public class TrustedHeaderAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            // Always clear UserContext at the end of request processing
            UserContextHolder.clearContext();
        }
    }

    /**
     * Process authentication headers and set SecurityContext and UserContextHolder.
     */
    private void processAuthHeaders(HttpServletRequest request) {
        ...
        String tokenId = request.getHeader(AuthHeaderConstants.HEADER_TOKEN_ID);
        ...
        // Store attributes for downstream use (e.g., TrustedTenantAccessFilter)
        ...
        if (userId != null && !userId.isBlank()) {
            try {
                ...
                request.setAttribute(AuthHeaderConstants.ATTR_USER_ID, parsedUserId);
        ...
        String clientIp = request.getHeader(AuthHeaderConstants.HEADER_CLIENT_IP);
        if (clientIp == null || clientIp.isBlank()) {
            clientIp = request.getRemoteAddr();
        }
        String userAgent = request.getHeader(AuthHeaderConstants.HEADER_USER_AGENT);
        if (userAgent == null || userAgent.isBlank()) {
            userAgent = request.getHeader("User-Agent");
        }

        // Set UserContext for audit and authorization
