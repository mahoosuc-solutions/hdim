...
## Next Action
Resolve the first-buyer question on March 12-13, 2026: Fallon-side quality leadership versus MGB integration leadership. Send T1 email on March 16, 2026 using the `merger-expansion` sequence with softened language for the pending regulatory review, framing HDIM as support for integration planning rather than post-close disruption. Immediately after send, log the T1 entry in the Outreach Status table, update [docs/abm/targets/_tracker.md](/mnt/wdblack/dev/projects/hdim-master/docs/abm/targets/_tracker.md), and set the next action to `T2 email due 2026-03-22`. Lead with the 4.5 to 4.0 MA Stars decline plus the measurement complexity of combining Fallon and MGB populations.
