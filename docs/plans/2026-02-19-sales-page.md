...
**Goal:** Create `landing-page-v0/app/sales/page.tsx` — a prospect-facing "how we work with you" page that builds buying confidence and routes visitors to `/schedule`.

**Architecture:** Single static Next.js page (App Router, no `'use client'` needed — no state). Six sections rendered server-side. Uses inline nav header matching the `/schedule` page pattern (no shared nav component exists for marketing pages). Tailwind CSS only, no new packages.

**Tech Stack:** Next.js 16 App Router, TypeScript, Tailwind CSS, lucide-react (already installed)

---

### Task 1: Create the page file with metadata + hero section

**Files:**
- Create: `landing-page-v0/app/sales/page.tsx`
...
```bash
cd landing-page-v0 && npm run build 2>&1 | grep -E "error|Error|✓|/sales"
```

Expected: `○ /sales` in the route list, no errors.

**Step 3: Commit**

```bash
git add landing-page-v0/app/sales/page.tsx
git commit -m "feat(sales): scaffold /sales page with hero section"
```

---

### Task 2: Add the 4-step process timeline section

**Files:**
- Modify: `landing-page-v0/app/sales/page.tsx` — replace `{/* rest of sections go here */}` comment

**Step 1: Define the steps data and add the section**

Add this data array before the `return` statement:
...
Add the section JSX inside `<main>` after the hero section:
...
```bash
cd landing-page-v0 && npm run build 2>&1 | grep -E "error|Error|✓"
```
...
```bash
git add landing-page-v0/app/sales/page.tsx
git commit -m "feat(sales): add 4-step process timeline section"
```

---

### Task 3: Add "What to Expect" timelines section

**Files:**
- Modify: `landing-page-v0/app/sales/page.tsx`

**Step 1: Add section after the process section**
...
```bash
git add landing-page-v0/app/sales/page.tsx
git commit -m "feat(sales): add What to Expect timelines section"
```

---

### Task 4: Add FAQ section (objection handling)

**Files:**
- Modify: `landing-page-v0/app/sales/page.tsx`

**Step 1: Add FAQ data before the return statement**
...
**Step 2: Add section JSX after the "What to Expect" section**
...
```bash
git add landing-page-v0/app/sales/page.tsx
git commit -m "feat(sales): add FAQ section with 6 objection-handling answers"
```

---

### Task 5: Add "Who This Is For" segment cards + final CTA section

**Files:**
- Modify: `landing-page-v0/app/sales/page.tsx`

**Step 1: Add segment data before return**
...
**Step 2: Add both the segments section and the final CTA section**
...
Expected: `○ /sales` in route list, no TypeScript errors.

**Step 4: Commit**

```bash
git add landing-page-v0/app/sales/page.tsx
git commit -m "feat(sales): add Who This Is For + final CTA sections — page complete"
```

---

### Task 6: Add /sales to sitemap + deploy

**Files:**
- Modify: `landing-page-v0/app/sitemap.ts` — add `/sales` entry
- Deploy to Vercel

**Step 1: Add to sitemap**

In `landing-page-v0/app/sitemap.ts`, add after the `/schedule` entry:
...
Expected: deployment URL aliased to `https://healthdatainmotion.com`, `/sales` visible in build output.

**Step 3: Smoke test**

```bash
curl -si https://www.healthdatainmotion.com/sales | head -5
# Expected: HTTP/2 200
```
...
```bash
git add landing-page-v0/app/sitemap.ts
git commit -m "feat(sales): add /sales to sitemap"
```
...
- [ ] `https://www.healthdatainmotion.com/sales` loads with HTTP 200
- [ ] All 6 sections visible: hero, process, timelines, FAQ, segments, CTA
- [ ] "Schedule Your Discovery Call" buttons link to `/schedule`
- [ ] `sales@mahoosuc.solutions` mailto link works
- [ ] `/sales` appears in sitemap XML
- [ ] No TypeScript errors in build output
- [ ] Mobile layout looks correct (stack to single column on small screens)
