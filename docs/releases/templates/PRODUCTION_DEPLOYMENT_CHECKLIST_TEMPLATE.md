# HDIM Platform Production Deployment Checklist - {VERSION}
...
- [ ] Review [RELEASE_NOTES_{VERSION}.md](./RELEASE_NOTES_{VERSION}.md)
- [ ] Review [UPGRADE_GUIDE_{VERSION}.md](./UPGRADE_GUIDE_{VERSION}.md)
- [ ] Review [KNOWN_ISSUES_{VERSION}.md](./KNOWN_ISSUES_{VERSION}.md)
- [ ] Review [VERSION_MATRIX_{VERSION}.md](./VERSION_MATRIX_{VERSION}.md)
...
./scripts/production-backup.sh {VERSION}

# Verify backups
ls -lh /backups/production/{VERSION}/
```
...
- [ ] Migration count matches expected: {EXPECTED_MIGRATION_COUNT}
...
```bash
# Pull latest images
docker pull hdim/gateway-service:{VERSION}
docker pull hdim/patient-service:{VERSION}
# ... (all 34 services)
