# HDIM Platform Production Deployment Checklist - v1.3.0
...
./scripts/production-backup.sh v1.3.0

# Verify backups
ls -lh /backups/production/v1.3.0/
```
...
```bash
# Pull latest images
docker pull hdim/gateway-service:v1.3.0
docker pull hdim/patient-service:v1.3.0
# ... (all 34 services)
...
### Step 7a: Verify New CQRS Event Services (v1.3.0)
```bash
...
kubectl logs -n hdim-production deployment/patient-event-service | grep "Kafka consumer group"
kubectl logs -n hdim-production deployment/care-gap-event-service | grep "Kafka consumer group"
kubectl logs -n hdim-production deployment/quality-measure-event-service | grep "Kafka consumer group"
kubectl logs -n hdim-production deployment/clinical-workflow-event-service | grep "Kafka consumer group"
```
