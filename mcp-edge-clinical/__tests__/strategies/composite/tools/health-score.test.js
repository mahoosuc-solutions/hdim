const { createDefinition } = require('../../../../lib/strategies/composite/tools/health-score');

describe('health_score tool', () => {
  ...
  it('has correct name', () => {
    expect(definition.name).toBe('health_score');
  });
  ...
  it('calls GET /quality-measure/patients/{patientId}/health-score', async () => {
    mockClient.get.mockResolvedValue({ status: 200, ok: true, body: '{"score":72}' });

    await definition.handler({ patientId: 'p1', tenantId: 'acme-health' });
    expect(mockClient.get).toHaveBeenCalledWith(
      '/quality-measure/patients/p1/health-score',
      { tenantId: 'acme-health' }
    );
  });

  it('encodes patientId in URL path', async () => {
    mockClient.get.mockResolvedValue({ status: 200, ok: true, body: '{}' });
    await definition.handler({ patientId: 'id with spaces', tenantId: 't1' });
    expect(mockClient.get).toHaveBeenCalledWith(
      '/quality-measure/patients/id%20with%20spaces/health-score',
      { tenantId: 't1' }
    );
  });
  ...
  it('returns response data correctly', async () => {
    mockClient.get.mockResolvedValue({ status: 200, ok: true, body: '{"score":72,"level":"moderate"}' });

    const result = await definition.handler({ patientId: 'p1', tenantId: 't1' });
    const parsed = JSON.parse(result.content[0].text);
    expect(parsed.status).toBe(200);
    expect(parsed.ok).toBe(true);
    expect(parsed.data).toBe('{"score":72,"level":"moderate"}');
  });
