const { createDefinition } = require('../../../../lib/strategies/composite/tools/measure-score');

describe('measure_score tool', () => {
  ...
  it('has correct name', () => {
    expect(definition.name).toBe('measure_score');
  });
  ...
  it('calls GET /quality-measure/score with patient query param', async () => {
    mockClient.get.mockResolvedValue({ status: 200, ok: true, body: '{"score":0.85}' });

    await definition.handler({ patientId: 'p1', tenantId: 'acme-health' });
    expect(mockClient.get).toHaveBeenCalledWith(
      '/quality-measure/score?patient=p1',
  ...
  it('encodes patientId in URL', async () => {
    ...
    expect(mockClient.get).toHaveBeenCalledWith(
      '/quality-measure/score?patient=id%20with%20spaces',
      { tenantId: 't1' }
    );
  });
  ...
  it('returns response data correctly', async () => {
    ...
    expect(parsed.data).toBe('{"score":0.85}');
  });
