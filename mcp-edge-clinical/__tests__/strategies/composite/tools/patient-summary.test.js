const { createDefinition } = require('../../../../lib/strategies/composite/tools/patient-summary');

describe('patient_summary tool', () => {
  ...
  it('has correct name', () => {
    expect(definition.name).toBe('patient_summary');
  });
  ...
  it('calls clinicalClient.get with correct path and tenantId', async () => {
    ...
    expect(mockClient.get).toHaveBeenCalledWith(
      '/patient/health-record?patient=p1',
    ...
    expect(parsed.data).toBe('{"resourceType":"Bundle"}');
  });
  ...
  it('encodes patientId in URL', async () => {
    ...
    expect(mockClient.get).toHaveBeenCalledWith(
      '/patient/health-record?patient=id%20with%20spaces',
      { tenantId: 't1' }
    );
  });
});
