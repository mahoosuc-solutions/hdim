...
parse_date_from_text() {
  local text="$1"
  if [[ "$text" =~ (20[0-9]{2}-[0-9]{2}-[0-9]{2}) ]]; then
    echo "${BASH_REMATCH[1]}"
    return 0
  fi
  return 1
...
check_freshness() {
  ...
  local date_str
  date_str="$(parse_date_from_text "$path" || true)"
  if [[ -z "$date_str" ]]; then
    date_str="$(date -u -r "$path" +%Y-%m-%d)"
  fi

  local now_epoch artifact_epoch age_days
  now_epoch="$(date -u +%s)"
  artifact_epoch="$(date -u -d "$date_str" +%s)"
  age_days=$(( (now_epoch - artifact_epoch) / 86400 ))

  if (( age_days > max_days )); then
    record "$label" "FAIL" "Stale: ${age_days}d old (max ${max_days}d), artifact=$path"
    failures=$((failures + 1))
  else
    record "$label" "PASS" "Fresh: ${age_days}d old (max ${max_days}d), artifact=$path"
  fi
}
...
# Document freshness windows by governance cadence.
...
# No-waiver critical controls must all be PASS.
critical_expected=6
